...
class PostCreate(BaseModel):
    ...
    # 최대 5장 이미지 제한 정책 적용
    image_urls: Optional[List[str]] = Field(default=[], max_length=5, description="첨부 이미지 URL 목록 (최대 5장)", json_schema_extra={"example": []})
...
class PostUpdate(BaseModel):
    """게시물 수정 시 전달받는 데이터 (전체 데이터 덮어쓰기 PUT 방식)"""
    title: NotEmptyStr = Field(..., max_length=255, description="수정할 게시물 제목")
    content: NotEmptyStr = Field(..., description="수정할 게시물 본문")
    movie_ids: List[UUID] = Field(..., min_length=1, description="수정할 공식 영화 ID 목록")
    image_urls: Optional[List[str]] = Field(default=[], max_length=5, description="수정할 첨부 이미지 URL 목록", json_schema_extra={"example": []})
    is_spoiler: int = Field(default=0, ge=0, le=1, description="스포일러 여부 (0: 일반, 1: 스포일러)")
